...
// Font height must be up to 16px (2 lines)
...
static uint16_t char_bitmap[32];

void my_txt(uint8_t x, uint8_t line, char *str)
{
  if (x >= SH1106_WIDTH || line >= (SH1106_HEIGHT / 16)) return;
  uint8_t *bufpos = sh1106_buffer + (SH1106_WIDTH * 2 * line) + x;
  ...
  while (*str) {
    if (*str >= 0x20 && *str <= 0x7E)
    {
      ...
      for (uint8_t row = 0; row < char_rows; row++)
      {
        ...
        row_shifts++;
        if (row_shifts == 16) row_shifts = 0;
      }

      // Go column by column two pages at a time
      for (col = 0; col < cols; col++) {
        *bufpos = (uint8_t)(char_bitmap[col] >> 8);
        *(bufpos + SH1106_WIDTH) = (uint8_t)(char_bitmap[col] & 0xFF);
        bufpos++;
      }
    }
    str++;
